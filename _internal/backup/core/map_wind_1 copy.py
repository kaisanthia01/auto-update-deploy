...
logger = logging.getLogger("MapWind_1")


class MapWind_1(QThread):
    text_update = pyqtSignal(str)  # สัญญาณที่ส่งข้อความไปยัง MainWindow
    finished = pyqtSignal(str)  # ✅ เพิ่ม signal finished พร้อมส่ง path รูป

    def __init__(self):
        super(MapWind_1, self).__init__()
        self.figure = None
        self.ax = None
        self.font = None
        ...
        self.json_data_upperwind_file = os.path.join(
            script_dir, "../data/json/synop_data_upperwind.json"
        )

        # ไฟล์ข้อมูลเปรียบเทียบสถานีแล้ว
        self.json_data_plot_upperwind_file = os.path.join(
            script_dir, "../data/json/synop_data_plot_upperwind_1.json"
        )
    ...
    def run(self, date=None, time=None, limit=None):
        try:
            ...
            output_file = os.path.join(script_dir, "../output/map/upperwind1.png")
            output_file_2 = os.path.join(
                script_dir, "../output/map/upperwind1-600dpi.png"
            )
            output_file_3 = os.path.join(script_dir, "../output/map/pdf/upperwind1.pdf")
            ...
            for idx, ax in enumerate(axes):
                if idx == 1:
                    ax.set_title(level_labels[idx])
                # ax.set_xticks([])  # ซ่อน tick x
                # ax.set_yticks([])  # ซ่อน tick y
            ...
            # แสดงข้อความที่ตำแหน่งลองจิจูด 65.0, ละติจูด -14.65
            # ax.text(
            #    65.0,  # ลองจิจูด (Longitude)
            #    -14.65,  # ละติจูด (Latitude)
            #    f"DATE: {date} TIME: {time} | อต.ทอ. 1002",  # ข้อความ
            #    transform=ccrs.PlateCarree(),  # ใช้พิกัดบนแผนที่
            #    fontsize=3,  # ขนาดตัวอักษร
            #    fontweight="bold",  # ตัวหนา
            #    fontproperties=font,  # ใช้ฟอนต์ที่โหลด
    ...
    def _add_obs_map(self, ax, date=None, time=None, limit=None, index_pressure=0):
        try:
            self.text_update.emit(
                f"🟡 Plot UpperWind1() Called - {index_pressure} hPa."
            )
            if not ax:
            ...
            try:
                ...
                self.text_update.emit("✅ Plot UpperWind1 สถานีด้วย MetPy")
                logger.info("✅ Plot UpperWind1 สถานีด้วย MetPy")

            except Exception as e:
                self.text_update.emit(f"❌ Plot UpperWind1 Error: {e}")
                logger.error(f"❌ Plot UpperWind1 Error: {e}")

        except Exception as e:
            self.text_update.emit(f"❌ Unexpected error in Plot UpperWind1: {e}")
            logger.error(f"❌ Unexpected error in Plot UpperWind1: {e}")
