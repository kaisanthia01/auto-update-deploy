...
class MainWindow(QMainWindow):
    ...
    def __init__(self, splash_bridge: Optional[SplashBridge] = None, vName: str = ""):
        ...
        self.check_cloud = 1  # ตัวนับเช็ครูปแบบการโหลดข้อมูล
    ...
    def plot_map_by_tab(
        ...
        if tab_name == "🗺️ Surface อต.ทอ. 1001":
            pm.plot_surface_and_update_qml(
                self.date_text, self.time_text, self.value_index, self.check_cloud
    ...
    def load_data(
        ...
        if tab_index == 0:
            ...
            try:
                ...
                # ✅ ตรวจสอบว่า cloud_text เป็นรูปแบบ black หรือ colors
                if cloud_text == "Black":
            ...
            if content != "NIL":
                ...
                if cloud_text == "Black":
                    try:
                        ...
                        self.check_cloud = 1  # โหลดข้อมูลแบบปกติ
                    ...
                    try:
                        ...
                        # self.surface_data.update({d["date"]: d for d in decoded})
                        for d in decoded:
                            date = d["date"]
                            if date not in self.surface_data:
                                self.surface_data[date] = []
                            self.surface_data[date].append(d)

                        self.check_cloud = 2  # โหลดข้อมูลแบบปกติ
            ...
            if content != "NIL":
                ...
                if cloud_text == "Black":
                    ...
                    try:
                        ...
                        decoded = decoder.decodeAAXX(content, index, save_to_file=True)
                        # self.detail_data.update({d["date"]: d for d in decoded})
                        for d in decoded:
                            date = d["date"]
                            if date not in self.detail_data:
                                self.detail_data[date] = []
                            self.detail_data[date].append(d)
    ...
    def table_view(self, tab_index: int = 0):
        ...
        try:
            ...
            if index in [0, 1, 2]:  # Surface / Pressure / Detail
                ...
                self.table_window = TableViewSynoptic(
                    tab_name=tab_name, data=data, check_cloud=self.check_cloud
                )
