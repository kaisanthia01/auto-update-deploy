...
Rectangle {
    ...
    property var mapNameList: ["surface", "pressure", "detail", "upperwind1", "upperwind2", "skewt"]
    property string surfaceImageMapPath: Qt.resolvedUrl("../../images/map/" + mapNameList[0] + ".png") // ✅ เริ่มต้นยังไม่มีเส้นทางไฟล์แผนที่
    property string pressureImageMapPath: Qt.resolvedUrl("../../images/map/" + mapNameList[1] + ".png") // ✅ เริ่มต้นยังไม่มีเส้นทางไฟล์แผนที่
    property string detailImageMapPath: Qt.resolvedUrl("../../images/map/" + mapNameList[2] + ".png") // ✅ เริ่มต้นยังไม่มีเส้นทางไฟล์แผนที่
    property string upperwind1ImageMapPath: Qt.resolvedUrl("../../images/map/" + mapNameList[3] + ".png") // ✅ เริ่มต้นยังไม่มีเส้นทางไฟล์แผนที่
    property string upperwind2ImageMapPath: Qt.resolvedUrl("../../images/map/" + mapNameList[4] + ".png") // ✅ เริ่มต้นยังไม่มีเส้นทางไฟล์แผนที่
    property string skewtImageMapPath: Qt.resolvedUrl("../../images/map/" + mapNameList[5] + ".png") // ✅ เริ่มต้นยังไม่มีเส้นทางไฟล์แผนที่

    property string surfaceOutputMapPath: Qt.resolvedUrl("../../output/map/" + mapNameList[0] + ".png") // ✅ เริ่มต้นยังไม่มี output map
    property string pressureOutputMapPath: Qt.resolvedUrl("../../output/map/" + mapNameList[1] + ".png") // ✅ เริ่มต้นยังไม่มี output map
    property string detailOutputMapPath: Qt.resolvedUrl("../../output/map/" + mapNameList[2] + ".png") // ✅ เริ่มต้นยังไม่มี output map
    property string upperwind1OutputMapPath: Qt.resolvedUrl("../../output/map/" + mapNameList[3] + ".png") // ✅ เริ่มต้นยังไม่มี output map
    property string upperwind2OutputMapPath: Qt.resolvedUrl("../../output/map/" + mapNameList[4] + ".png")  // ✅ เริ่มต้นยังไม่มี output map
    property string skewtOutputMapPath: Qt.resolvedUrl("../../output/map/" + mapNameList[5] + ".png") // ✅ เริ่มต้นยังไม่มี output map

    property bool isDataCheckURL: false  // ✅ เริ่มต้นยังไม่ได้ตรวจสอบ URL
    property bool isDataLoaded: false  // ✅ เริ่มต้นยังไม่ได้โหลด
    property bool isPlotMap: false  // ✅ เริ่มต้นยังไม่ได้ Plot
    property bool isConvertoA3: false  // ✅ เริ่มต้นยังไม่ได้ Convert file
    ...
    property bool plotSkewtMap: false // ✅ สถานะการแสดงผลแผนที่
    ...
    Connections {
        ...
        function onDataLoadedChanged(loaded) {
            ...
            msgDialog.message = loaded ? "🌏 Data loaded successfully!\n" + tabBar.currentItem.text : "❌ Failed to load data.\n" + tabBar.currentItem.text;
            msgDialog.open();  // ✅ เปิด Dialog อย่างถูกต้อง
        }
    }
    ...
    Connections {
        target: rootWindow
        function onConvertA3(loaded) {
            let current = debugTextArea.text;
            debugTextArea.text = "";         // ล้างชั่วคราว
            debugTextArea.text = current;    // ใส่คืน

            root.isConvertoA3 = loaded;
            msgDialog.message = loaded ? "🔄 Coverto File Successfully!\n" + tabBar.currentItem.text : "❌ Failed to convert file.\n" + tabBar.currentItem.text;
            msgDialog.open();  // ✅ เปิด Dialog อย่างถูกต้อง
        }
    }
        ...
        GridLayout {
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    // 🎚️ Radio Button Pair Cloud Sureface and Detail
                    Label {
                        text: "🌦️ Cloud Colors:"
                        color: "#fff"
                        font.pixelSize: 14
                        Layout.alignment: Qt.AlignLeft
                        visible: (root.currentIndex === 0 || root.currentIndex === 2) && root.isDataCheckURL === true // ✅ แสดงเฉพาะ tab Surface and Detail
                    }
                    ...
                    Label {
                        ...
                        opacity: dataSliderText.enabled ? 1.0 : 0.5  // ✅ ทำให้จางลงถ้ากดไม่ได้
                    }
                    ...
                    Slider {
                        ...
                        background: Rectangle {
                            ...
                            radius: 2
                            color: "#bdbebf"
                            opacity: dataSlider.enabled ? 1.0 : 0.5  // ✅ ทำให้จางลงถ้ากดไม่ได้
                    ...
                    // 🎚️ Radio Button Pair Pressure SkewT
                    Label {
                        text: "🎚 Pressure Level:"
                        color: "#fff"
                        font.pixelSize: 14
                        ...
                        Control {
                            ...
                            Timer {
                                ...
                                onTriggered: {
                                    ...
                                    let radioButtonChecked = blackRadio.checked ? "Black" : "Colors";

                                    if (tabIndex === 5) {
                                        rootWindow.load_data(selectedDate, selectedTime, "", station, tabIndex);
                                    } else {
                                        rootWindow.load_data(selectedDate, selectedTime, radioButtonChecked, "", tabIndex);
                                    }
                                }
                            }
                        }
                    ...
                    RowLayout {
                        ...
                        Control {
                            ...
                            implicitHeight: 30
                            visible: root.currentIndex === 5  // ✅ แสดงเฉพาะ tab Skew-T
                            enabled: root.isPlotMap  // ✅ ใช้ property นี้

                            background: Rectangle {
                                color: pdfButton.hovered ? "#318b00" : "#599d34"  // สี hover
                                radius: 6
                                opacity: pdfButton.enabled ? 1.0 : 0.5  // ✅ ทำให้จางลงถ้ากดไม่ได้
                            }
                            ...
                            MouseArea {
                                anchors.fill: parent
                                enabled: pdfButton.enabled  // ✅ ตรงนี้สำคัญมาก
                                cursorShape: Qt.PointingHandCursor
                                hoverEnabled: true
                        ...
                        Control {
                            ...
                            background: Rectangle {
                                ...
                                radius: 6
                                opacity: pngButton.enabled ? 1.0 : 0.5  // ✅ ทำให้จางลงถ้ากดไม่ได้
                            }
                            ...
                            MouseArea {
                                anchors.fill: parent
                                enabled: pngButton.enabled  // ✅ ตรงนี้สำคัญมาก
                                cursorShape: Qt.PointingHandCursor
                                hoverEnabled: true
                    ...
                    // ⏳ Convert to PDF A3 + 📄 Save File PDF A3
                    RowLayout {
                        Layout.columnSpan: 2
                        spacing: 10
                        ...
                        Control {
                            ...
                            Layout.columnSpan: 2
                            implicitHeight: 30
                            visible: root.currentIndex === 3 || root.currentIndex === 4  // ✅ ปรับตรงนี้
                            enabled: root.isPlotMap  // ✅ ใช้ property นี้

                            background: Rectangle {
                                color: convertButton.hovered ? "#318b00" : "#599d34"  // สี hover
                                radius: 6
                                opacity: convertButton.enabled ? 1.0 : 0.5  // ✅ ทำให้จางลงถ้ากดไม่ได้
                            }
                            ...
                            MouseArea {
                                anchors.fill: parent
                                enabled: convertButton.enabled  // ✅ ตรงนี้สำคัญมาก
                                cursorShape: Qt.PointingHandCursor
                                hoverEnabled: true
                                onEntered: convertButton.hovered = true
                                onExited: convertButton.hovered = false
                                //onClicked: rootWindow.buttonClicked("pdf")
                                onClicked: {
                                    msgDialog.message = "⏳ Waiting Convert File...!\n" + tabBar.currentItem.text;
                                    msgDialog.open();  // ✅ เปิด Dialog ก่อน

                                    // ✅ รอให้ msgDialog แสดงก่อน แล้วค่อยโหลดข้อมูล
                                    convertDelayTimer.start();
                                }
                            }

                            // วางไว้ใน scope เดียวกับ MouseArea (เช่นใน Item เดียวกัน)
                            Timer {
                                id: convertDelayTimer
                                interval: 100  // หน่วง 100 มิลลิวินาที
                                repeat: false
                                running: false
                                onTriggered: rootWindow.convert_a3(tabBar.currentIndex)
                            }
                        }
                        ...
                        Control {
                            ...
                            background: Rectangle {
                                color: pdfA3Button.hovered ? "#0051e7" : "#2064e2"  // สี hover
                                radius: 6
                                opacity: pdfA3Button.enabled ? 1.0 : 0.5  // ✅ ทำให้จางลงถ้ากดไม่ได้
                            }
                            ...
                            MouseArea {
                                anchors.fill: parent
                                enabled: pdfA3Button.enabled  // ✅ ตรงนี้สำคัญมาก
                                cursorShape: Qt.PointingHandCursor
                                hoverEnabled: true
                                onEntered: pdfA3Button.hovered = true
                                onExited: pdfA3Button.hovered = false
                                //onClicked: rootWindow.buttonClicked("png:" + tabBar.currentIndex)
                                onClicked: {
                                    let selectedDate = datePicker.date.toLocaleDateString(Qt.locale("en_US"), "yyyy-MM-dd");
                                    let selectedTime = timeSelector.currentText;

                                    rootWindow.save_a3(selectedDate, selectedTime, tabBar.currentIndex);
                                }
                            }
                        }
                    }
