...
logger = logging.getLogger("CheckURLDetail2day")
...
class CheckURLDetail2day:
    ...
    def _extract_synop(self, raw_text: str) -> str:
        ...
        lines = [line for line in lines if line.startswith(target_prefixes)]

        # ✅ เก็บเฉพาะสถานีล่าสุด
        unique = {}
        for entry in lines:
    ...
    def save_json(self, data: list):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))  # ตำแหน่งไฟล์ .py
        output_path = os.path.join(script_dir, "../data/json/synop_url_detail2day.json")
