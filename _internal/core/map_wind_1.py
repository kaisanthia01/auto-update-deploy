...
class MapWind_1(QThread):
    ...
    def __init__(self):
        ...
        self.json_data_upperwind_file = os.path.join(
            script_dir, "../data/json/synop_data_upperwind_1.json"
        )
    ...
    def run(self, date=None, time=None, limit=None):
        try:
            ...
            ax.text(
                -0.7,
                2.147,  # ตำแหน่งตรงกลางด้านบน (x=0.5 คือกลาง, y>1 คือเหนือ plot)
                f"{date.upper()}",
    ...
    def _add_obs_map(self, ax, date=None, time=None, limit=None, index_pressure=0):
        try:
            ...
            try:
                ...
                # ▶️ ลูกศรลม
                plot.plot_arrow(
                    np.nan_to_num(u_fixed.values),  # ความเร็วลมแนวตะวันออก-ตะวันตก
                    np.nan_to_num(v_fixed.values),  # ความเร็วลมแนวเหนือ-ใต้
                    scale=2.5,  # ขนาดลูกศร (เล็ก = เลขมาก, ใหญ่ = เลขน้อย)
                    scale_units="inches",  # หน่วยของสเกล ใช้ร่วมกับ inches
                    width=0.0015,  # ความหนาของลูกศร
                    pivot="middle",  # จุดหมุน (tail หัวลูกศรชี้ออกจากจุดเริ่มต้น/ middle จุดศูนย์กลางของลูกศรอยู่ที่จุดข้อมูล/ tip 	ปลายลูกศรอยู่ที่จุดข้อมูล)
                    color="red",  # สีลูกศร
                    zorder=10,  # ลำดับการวาด (สูงกว่าค่าปกติจะอยู่ด้านบน)
                )
