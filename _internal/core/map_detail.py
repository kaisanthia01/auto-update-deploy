...
class MapDetail(QThread):
    ...
    def __init__(self):
        ...
        self.json_data_detail_file = os.path.join(
            script_dir, "../data/json/synop_data_detail.json"
        )

        self.json_data_detail2day_file = os.path.join(
            script_dir, "../data/json/synop_data_detail2day.json"
        )

        # ไฟล์ข้อมูลเปรียบเทียบสถานีแล้ว
        self.json_data_plot_detail_file = os.path.join(
            script_dir, "../data/json/synop_data_plot_detail.json"
        )
    ...
    def run(self, date=None, time=None, limit=None, check_cloud=1):
        try:
            ...
            output_file_4 = os.path.join(
                script_dir, "../output/map/" + map_name + "_297x420mm_300dpi.png"
            )
            ...
            ax.text(
                ...
                f"Date: {date} Time: {time[0:8]} | Detail Map",  # ข้อความ
            ...
            # === เพิ่มตัวอักษรบนแผนที่ ===
            # แสดงข้อความที่ตำแหน่งลองจิจูด 99.0, ละติจูด -2.0
            ax.text(
                99.0,  # ลองจิจูด (Longitude)
                -2.0,  # ละติจูด (Latitude)
                f"{date.upper()}",  # ข้อความ
                transform=ccrs.PlateCarree(),  # ใช้พิกัดบนแผนที่
            ...
            # === เพิ่มตัวอักษรบนแผนที่ ===
            # แสดงข้อความที่ตำแหน่งลองจิจูด 105.0, ละติจูด -2.0
            ax.text(
            ...
            # ขนาดเป้าหมาย (297 x 420 mm ที่ 300 DPI)
            target_size = (3508, 4961)
    ...
    def _add_obs_map(self, ax, date=None, time=None, limit=None, check_cloud=1):
        # ✅ ตรวจสอบว่า check_cloud เป็นรูปแบบ black หรือ colors
        if check_cloud == 1:
            try:
                self.text_update.emit("🟡 Plot Detail() Called")
                logger.info("🟡 Plot Detail() Called")
                ...
                logger.info(f"📄 Loaded parsed detail data: {len(data_synop)} entries")
                ...
                if len(dates) > 0:
                    ...
                    logger.error("❌ ต้องการข้อมูลอย่างน้อย 1 วัน")
                    return ax
                ...
                try:
                    self.text_update.emit("▶️ Plot Detail สถานีด้วย MetPy")
                    logger.info("▶️ Plot Detail สถานีด้วย MetPy")
                    ...
                    self.text_update.emit("✅ Plot Detail สถานีด้วย MetPy")
                    logger.info("✅ Plot Detail สถานีด้วย MetPy")
                except Exception as e:
                    self.text_update.emit(f"❌ Plot Detail Error: {e}")
                    logger.error(f"❌ Plot Detail Error: {e}")
            except Exception as e:
                self.text_update.emit(f"❌ Unexpected error in Plot Detail: {e}")
                logger.exception("❌ Unexpected error in Plot Detail")

        else:
            try:
                self.text_update.emit("🟡 Plot Detail Colors() Called")
                logger.info("🟡 Plot Detail Colors() Called")
                ...
                # โหลดข้อมูล JSON เฉพาะ detail 2 day เท่านั้น
                data_synop = self.load_json(self.json_data_detail2day_file)
                self.text_update.emit(
                    f"📄 Loaded parsed detail 2 day data: {len(data_synop)} entries"
                )
                ...
                try:
                    self.text_update.emit("▶️ Plot Detail Colors สถานีด้วย MetPy")
                    logger.info("▶️ Plot Detail Colors สถานีด้วย MetPy")
                    ...
                    self.text_update.emit("✅ Plot Detail Colors สถานีด้วย MetPy")
                    logger.info("✅ Plot Detail Colors สถานีด้วย MetPy")
                except Exception as e:
                    self.text_update.emit(f"❌ Plot Detail Colors Error: {e}")
                    logger.error(f"❌ Plot Detail Colors Error: {e}")
            except Exception as e:
                self.text_update.emit(f"❌ Unexpected error in Plot Detail Colors: {e}")
                logger.exception("❌ Unexpected error in Plot Detail Colors")
