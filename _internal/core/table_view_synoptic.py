...
class TableViewSynoptic(QWidget):
    def __init__(self, tab_name="", data=None, check_cloud=1):
        super().__init__()

        try:
            self.setWindowTitle(f"Table View Synoptic - {tab_name}")
            self.setWindowIcon(
                QIcon(os.path.join(os.path.dirname(__file__), "..", "icons", "app.ico"))
            )
            ...
            self.table_view.sortByColumn(
                self.headers.index("station_id"), Qt.AscendingOrder
            )
    ...
    def remove_selected_row(self):
        ...
        if self.tabName == "Surface อต.ทอ. 1001":
            output_path = os.path.join(
                ...
                "../data/json/synop_data_surface" + cloud + ".json",
            )
            ...
            output_path = os.path.join(
                os.path.dirname(__file__),
                "../data/json/synop_data_detail" + cloud + ".json",
            )
