...
logger = logging.getLogger("CheckURLDetail")
...
class CheckURLDetail:
    ...
    def _extract_synop_text(self, html_text):
        """แยกข้อมูล SYNOP รายสถานีจาก HTML"""
        ...
        for line in cleaned_text.splitlines():
            ...
            if line[:1].isdigit():
                if "=" in line:
                    synop_data += line.strip() + "\n"
                else:
                    synop_data += line.strip() + " "

        # ✅ กรองเฉพาะรหัสสถานีไทย
        target_prefixes = ("48", "56", "59", "96")
        filtered_lines = [
            line for line in synop_data.splitlines() if line.startswith(target_prefixes)
        ]

        # ✅ เก็บเฉพาะสถานีล่าสุด (โดยใช้ station_id เป็น key)
        unique = {}
        for entry in filtered_lines:
            if len(entry) >= 5 and entry[:5].isdigit():
                station_id = entry[:5]
                unique[station_id] = entry.strip()
    ...
    def urlGetContent(self, time, date, html_text=None):
        ...
        try:
            ...
            result, status = self._extract_synop_text(html)
            if result is None:
    ...
    def _save_to_json(self, synop_text: str, date: str):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))  # ตำแหน่งไฟล์ .py
        json_path = os.path.join(script_dir, "../data/json/synop_url_detail.json")
